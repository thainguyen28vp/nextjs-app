...
export default function ChartTop() {
                ...
                <div className="space-y-2">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <div key={i} className="flex items-center gap-3">
                            ...
                            <Skeleton className="h-6 rounded-md" style={{ width: `${60 - i * 8}%` }} />
                        </div>
                    ))}
                </div>
            </div>
    ...
    const displayData = chartData.map((d) => ({
        ...d,
        shortName: d.TEN_CHI_TIEU.length > 22
    ...
    return (
        <div className="rounded-2xl border border-border/50 bg-card p-4 shadow-sm">
            <div className="mb-4">
                <h2 className="text-base font-semibold text-foreground">Top sản phẩm / dịch vụ</h2>
                <p className="text-xs text-muted-foreground">Theo doanh thu</p>
            </div>

            <ChartContainer config={chartConfig} className="w-full" style={{ height: displayData.length * 48 + 24 }}>
                <BarChart
                    data={displayData}
                    layout="vertical"
